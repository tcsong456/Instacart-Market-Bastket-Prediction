...
def test_create_bronze_datasets_pipeline(spark, tiny_fake_testset):
    build_bronze_datasets(
        spark=spark, input_dir=tiny_fake_testset, output_dir=tiny_fake_testset
    )

    orders = read_parquet(tiny_fake_testset / "orders", spark)
    ...
    prior = read_parquet(tiny_fake_testset / "order_products__prior", spark)
    ...
    train = read_parquet(tiny_fake_testset / "order_products__train", spark)
    ...
    products = read_parquet(tiny_fake_testset / "products", spark)
    ...
    aisles = read_parquet(tiny_fake_testset / "aisles", spark)
    ...
    departments = read_parquet(tiny_fake_testset / "departments", spark)
